...
export default function tasks_index() {
  ...
  useEffect(() => { // Инициализация уведомлений
    notificationsInit();
  }, []);
  ...
  const onDateChange = (event, selectedDate) => { // Изменение даты
  ...
  const onTimeChange = (event, selectedDate) => { // Изменение времени
    const currentTime = new Date(selectedDate);
    currentTime.setDate(taskDate.getDate());
    setTaskDate(new Date(currentTime));
    setShowTimePicker(false);
  };
  const createTask = async (text: string, date: Date) => { // Создание пуш-уведомления
  ...
  return (
    <ScrollView style={styles.scrollView} contentContainerStyle={styles.container}>
      <Text style={adaptiveStyle.title}>Создайте новое напоминание</Text>
      <View style={styles.info}>
        <View style={adaptiveStyle.infoblock}>
          <Text style={adaptiveStyle.h2}>Важно!</Text>
          <Text style={adaptiveStyle.text_infoblock}>
        ...
        <View style={adaptiveStyle.example}>
          <Text style={styles.example_title}>📌 LavaNote напоминает!</Text>
          <Text style={adaptiveStyle.text}>{taskTitle}</Text>
        </View>
        <Text style={adaptiveStyle.text}>{`Сигнал прозвучит: ${getDisplayDate(taskDate.getTime())}`}</Text>
      </View>
